...
get_page_insights <- function(pageid, timeframe = "LAST_30_DAYS", lang = "en-GB",
                              ...
                              join_info = TRUE, max_consecutive_errors = 3, local = TRUE,
                              verbose = TRUE) 
{
  if (verbose) message("🔹 Starting `get_page_insights` for Page ID: ", pageid, ", Country: ", iso2c)
  ...
  if (verbose) message("✅ Current consecutive error count: ", consecutive_error_count)
  
  # Exit early if too many errors
  if (consecutive_error_count >= max_consecutive_errors) {
    if (verbose) message("❌ Max consecutive errors reached. Exiting early...")
    return(tibble())
  }
  ...
  if (verbose) message("✅ API request completed. Processing response...")
  ...
  if (!is.null(out[[1]][["errors"]][["description"]])) {
    consecutive_error_count <- get("consecutive_error_count", envir = .GlobalEnv) + 1
    ...
    return(tibble(error = out[[1]][["errors"]][["description"]]))
  }
  
  # Reset error count on success
  assign("consecutive_error_count", 0, envir = .GlobalEnv)
  if (verbose) message("✅ API response successful. Extracting data...")
  
  # Extracting Page Info
  if ("page_info" %in% include_info) {
    if (verbose) message("📌 Extracting Page Info...")
    
    page_info <- out[[1]][["data"]][["ad_library_page_info"]][["page_info"]]
    
    if (is.null(page_info)) {
      if (verbose) message("⚠️ No Page Info found. Returning empty dataframe.")
      
      return(tibble(page_id = pageid, no_data = TRUE))
    }
    
    page_info_df <- as.data.frame(t(unlist(page_info)), stringsAsFactors = FALSE) %>%
      mutate_all(as.character)
  }
  
  # Extracting Targeting Info
  if ("targeting_info" %in% include_info) {
    if (verbose) message("📌 Extracting Targeting Info...")
    
    targeting_info <- out[[1]][["data"]][["page"]][["ad_library_page_targeting_insight"]]
    
    if (!is.null(targeting_info)) {
      targeting_info <- targeting_info %>%
        discard(is_empty) %>%
        imap_dfr(~ .x %>% mutate(type = .y %>% str_remove("ad_library_page_targeting_"))) %>%
        mutate(page_id = pageid)
    } else {
      if (verbose) message("⚠️ No Targeting Info found.")
      targeting_info <- tibble(page_id = pageid, no_data = TRUE)
    }
  }
  
  # Merging Data
  if ("page_info" %in% include_info & "targeting_info" %in% include_info) {
    if (verbose) message("📌 Merging Page Info and Targeting Info...")
    
    if (join_info) {
      final_data <- left_join(page_info_df, targeting_info, by = "page_id")
    } else {
      final_data <- list(page_info = page_info_df, targeting_info = targeting_info)
    }
  } else if ("page_info" %in% include_info) {
    final_data <- page_info_df
  } else if ("targeting_info" %in% include_info) {
    final_data <- targeting_info
  }
  
  if (verbose) message("✅ Data extraction complete. Returning results.")
  
  return(final_data)
}

get_page_insights_old <- function(pageid, timeframe = "LAST_30_DAYS", lang = "en-GB",
                              iso2c = "US", include_info = c("page_info", "targeting_info"),
                              join_info = T, max_consecutive_errors = 3, local = T) 
{
  # Ensure global error counter exists
  if (!exists("consecutive_error_count", envir = .GlobalEnv)) {
    assign("consecutive_error_count", 0, envir = .GlobalEnv)
